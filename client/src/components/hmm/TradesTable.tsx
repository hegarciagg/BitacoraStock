  ...
  if (!data || data.length === 0) return (
    <Card className="bg-slate-800 border-slate-700 border-dashed">
      <CardContent className="py-12 text-center">
        ...
        <p className="text-slate-400 text-sm">Sin trades. Ejecuta un backtest para ver el historial.</p>
      </CardContent>
    </Card>
  );
    ...
    <Card className="bg-slate-800 border-slate-700">
      <CardHeader>
        <CardTitle className="text-white flex items-center gap-2">
          ...
          Historial de Trades
          <span className="text-xs text-slate-400 font-normal ml-auto">{data.length} trades</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-slate-700">
                <th className="py-2 px-3 text-left text-slate-400 font-semibold">Entrada</th>
                <th className="py-2 px-3 text-left text-slate-400 font-semibold">Salida</th>
                <th className="py-2 px-3 text-right text-slate-400 font-semibold">PnL</th>
                <th className="py-2 px-3 text-right text-slate-400 font-semibold">Régimen</th>
                <th className="py-2 px-3 text-right text-slate-400 font-semibold">Votos</th>
                <th className="py-2 px-3 text-right text-slate-400 font-semibold">Capital Final</th>
              </tr>
            </thead>
            <tbody>
              {data.map((trade) => {
                ...
                return (
                  <tr key={trade.id} className="border-b border-slate-700/50 hover:bg-slate-700/20 transition-colors">
                    ...
                    <td className="py-2 px-3 text-right text-slate-300">{trade.confirmations}/8</td>
