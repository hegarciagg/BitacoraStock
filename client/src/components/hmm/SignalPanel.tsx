...
export default function SignalPanel({ symbol = "BTC-USD" }: SignalPanelProps) {
    ...
    <Card className="bg-white shadow-sm border-slate-200">
      <CardContent className="pt-8 pb-8 flex items-center justify-center">
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-500 text-sm">Ejecutando HMM... esto puede tardar ~30s</p>
        </div>
      </CardContent>
    </Card>
  );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {/* Main Signal */}
      <Card className={`md:col-span-1 bg-white shadow-sm border-2 ${isLong ? "border-green-500/50" : "border-slate-200"}`}>
        <CardHeader className="pb-2">
          <CardTitle className="text-slate-900 text-sm flex items-center gap-2">
            {isLong ? <TrendingUp className="w-4 h-4 text-green-400" /> : <TrendingDown className="w-4 h-4 text-slate-500" />}
            Señal Actual
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className={`text-5xl font-black mb-3 ${isLong ? "text-green-400" : "text-slate-500"}`}>
            {data.signal}
          </div>
          <p className="text-xs text-slate-500 leading-relaxed">{data.reason}</p>
          {data.lastPrice && (
            <div className="mt-3 pt-3 border-t border-slate-200">
              <p className="text-xs text-slate-500">Precio {data.symbol ?? symbol}</p>
              <p className="text-slate-900 font-bold">${data.lastPrice.toLocaleString("en-US", { maximumFractionDigits: 2 })}</p>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="bg-white shadow-sm border-slate-200">
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs text-slate-500">Régimen Actual</span>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-bold text-slate-900">Estado {data.bullState}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-xs text-red-400">🔴 Bear State</span>
            <span className="text-sm font-bold text-slate-900">Estado {data.bearState}</span>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          ...
          <div className="mb-4">
            <div className="flex justify-between mb-1">
              ...
              <span className={`text-sm font-bold ${data.votingScore >= 7 ? "text-green-400" : "text-slate-500"}`}>
                {data.votingScore}/8
              </span>
            </div>
          ...
          {data.checks && (
            <div className="grid grid-cols-2 gap-1">
              {Object.entries(data.checks).map(([key, val]) => (
                <div key={key} className={`text-xs px-2 py-1 rounded ${val ? "bg-green-500/10 text-green-400" : "bg-slate-50 border border-slate-200 text-slate-500"}`}>
          ...
          {/* Cooldown */}
          {data.cooldownUntil && (
            <div className="mt-3 pt-3 border-t border-slate-200 text-xs text-yellow-400">
